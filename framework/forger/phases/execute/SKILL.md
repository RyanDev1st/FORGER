...
`acceptance_results.jsonl` and the Stop hook `hooks/enforce_done_means_ran.mjs`.
...
- `workspaces/{slug}/failure_hypotheses.yaml` — read-only. Every
  ...
  DoW criteria.
- Mode config — `modes/{quick|standard|deep}.yaml`. Sets the retry
...
- `workspaces/{slug}/acceptance_results.jsonl` — one JSON line per
  ...
  hook `hooks/enforce_done_means_ran.mjs` consumes the latest line
  per `criterion_id`.
...
3. **No `dow.unacceptable_failure_modes` triggers in detection runs.**
   ...
   *not* detected) for the line to be marked `passed: true`. This
   inversion is implemented inside `gates/acceptance_test.mjs`.
4. **Done Means Ran (invariant 1) — `hooks/enforce_done_means_ran.mjs`
...
3. **GREEN.** Write the minimal code to pass the test. Save the file.
   The PostToolUse hook `hooks/post_code.mjs` fires automatically
...
5. Invoke `gates/acceptance_test.mjs --workspace <path>`. The gate
...
Loop until `gates/acceptance_test.mjs` exits 0.
...
3. Run `gates/audit.mjs --workspace <path>` on the report. The audit
...
4. Invoke `gates/acceptance_test.mjs --workspace <path>` to write
...
2. Acceptance criteria for designs include **rubric scoring**
   ...
   walkthrough).
3. Invoke `gates/acceptance_test.mjs --workspace <path>` to write
...
Triggered when `gates/acceptance_test.mjs` fails AND the agent
...
`gates/acceptance_test.mjs` exits 0 (all required criteria pass) and
`hooks/enforce_done_means_ran.mjs` does not block on the
...
- `gates/acceptance_test.mjs` — appends `acceptance_results.jsonl`
  lines per DoW criterion; exits 0 when all required criteria pass.
- `gates/audit.mjs` — re-run on research_report branch and as the
