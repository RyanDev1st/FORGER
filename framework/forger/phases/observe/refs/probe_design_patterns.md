...
assumption via `tools/probe.mjs`. The tool accepts exactly six probe
types:
...
The tool invocation is always:
  ...
  `node tools/probe.mjs --workspace <path> --assumption-id <asm-id> --type <type> --cmd "<command>" [--timeout-ms <N>]`
