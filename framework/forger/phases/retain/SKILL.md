...
`retro_note.yaml` — and you invoke `tools/update_kb.mjs` once. The tool
...
exit from `tools/update_kb.mjs`. The KB mutations are durable; the
...
- Mode config — `modes/{quick|standard|deep}.yaml`. The mode label is
...
- **Mutations to `knowledge/{domain_slug}/`** performed by
  `tools/update_kb.mjs`. The tool merges proven claims into
...
1. **`retro_note.yaml` schema validates.** Run
   `_lib/ledger.mjs::validateRetroNote` against the file before
   invoking `tools/update_kb.mjs`. The tool re-validates on its way in
   and throws on invalid input; failing the schema means the retro is
   wrong, not that the tool is wrong.
2. **`tools/update_kb.mjs` returns exit 0.** Any non-zero exit means
...
`tools/update_kb.mjs` exits 0 and `retro_note.yaml` validates. There is
...
- `refs/kb_write_rules.md` — what qualifies as a proven claim, a
  ...
  to a specific check inside `tools/update_kb.mjs`.
...
- `refs/shortcut_eligibility.md` — how a domain becomes shortcut-
  ...
  flag interacts with `tools/update_kb.mjs`'s last-3 status check.
- `tools/update_kb.mjs` — performs every KB write; reads the retro,
...
- `_lib/ledger.mjs` — exports `validateRetroNote` and the YAML/JSONL
