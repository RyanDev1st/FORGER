...
- `knowledge/{domain_slug}/` — read-only knowledge base for this domain.
  ...
  shortcut available).
- `skills/forger/modes/{mode}.yaml` — read to discover which lanes to spawn (the `lanes`
  list). See `skills/forger/modes/quick.yaml`, `skills/forger/modes/standard.yaml`, `skills/forger/modes/deep.yaml`.
...
1. **AJV validation of both ledgers passes.** Use `src/lib/ledger.mjs`
...
4. **`src/gates/audit.mjs` passes.** HEAD, quote-grep, schema, lineage,
...
`skills/forger/modes/{mode}.yaml` to get the `lanes` list (this is the set of lanes you
...
- If coverage ≥ 0.80: copy the cached entries into the workspace ledgers,
  ...
  `find_summary.md`. **Skip steps 3-6.** Jump to step 7's audit (`src/gates/audit.mjs`)
...
- the effort budget from `skills/forger/modes/{mode}.yaml` (`token_budget_cold`)
...
- **V5.** AJV schema validation passes for every entry the lane wrote.
  Call `validateSourceEntry` and `validateClaimEntry` from `src/lib/ledger.mjs`.
...
- Run the same audit (`src/gates/audit.mjs`) and the same blocked-claim
  tagging.
...
- `refs/pivot_procedure.md` — 5-step pivot procedure for under-sourced
  topics.
- `src/lib/kb.mjs` — KB-shortcut helpers (`pruneExpired`, `computeCoverage`,
  `isShortcutEligible`).
- `src/lib/ledger.mjs` — schema validators (`validateSourceEntry`,
  `validateClaimEntry`).
- `src/gates/audit.mjs` — final audit (HEAD + quote-grep + lineage + bigram +
  independence).
- `src/cli/filter.mjs` — mechanical pre-filter used inside each lane.
