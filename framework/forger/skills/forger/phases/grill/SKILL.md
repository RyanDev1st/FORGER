...
name: forger-grill
description: |
  Phase 4 of FORGER. Cross-model adversarial review via src/lib/reviewer_router.
...
family* via `src/lib/reviewer_router.mjs`, parsing its concerns into
...
- Mode config — sourced from `skills/forger/modes/{quick|standard|deep}.yaml`.
  Three fields drive this phase: `grill_required` (quick can skip
  entirely), `grill_blind_reviewer` (deep adds the blind pass),
  `grill_min_reviewer_tier` (the floor `src/lib/reviewer_router.mjs`
  must meet).
...
1. **AJV validates `failure_hypotheses.yaml`.** Every list entry
   passes `src/lib/ledger.mjs::validateFailureHypothesis`. A schema miss
...
3. **`src/gates/audit.mjs` re-runs on all artifacts; no new audit
...
4. **`reviewer_tier` recorded in every failure_hypothesis entry;
   ...
   `src/lib/reviewer_router.mjs`. The deep-mode blind constraint is
...
Run `src/gates/audit.mjs --workspace <path>` to confirm the artifacts the
...
Call `src/lib/reviewer_router.mjs::invokeReviewer(systemPrompt, userPrompt, {blind: false})`.
...
1. `src/lib/ledger.mjs::validateFailureHypothesis` returns `{ valid: true }`
   for every entry.
...
- `src/gates/audit.mjs` — re-run in step 1 and gate 3.
- `src/lib/reviewer_router.mjs` — `invokeReviewer` plus tier semantics
