...
brief, you write `risk_map.yaml`, and you drive `src/cli/probe.mjs` until
...
- `workspaces/{slug}/risk_map.yaml` — must validate against
  `schemas/risk_map.schema.yaml` via `src/lib/ledger.mjs::validateRiskMap`.
  Contains `mechanisms[]`, `known_failure_modes[]`, and `assumptions[]`.
- `workspaces/{slug}/probe_results.jsonl` — append-only log of every
  probe `src/cli/probe.mjs` runs. The tool validates each line against
...
1. **`risk_map.yaml` schema validates.** `src/lib/ledger.mjs::validateRiskMap`
...
- `src/cli/probe.mjs` — the probe runner. Sandboxes commands per
...
- `src/lib/ledger.mjs` — `validateRiskMap`, `readYaml`, `writeYaml`,
  `appendProbeResult`.
