...
- Mode's `recombine_tiers_allowed` list, sourced from
  `skills/forger/modes/{quick|standard|deep}.yaml`. Quick allows `[1]` only;
...
4. **No Tier 2/3 content is written outside its dedicated file.**
   Enforced by `src/hooks/enforce_tier_firewall.mjs` (PreToolUse): the
...
2/3 content. The PreToolUse hook (`src/hooks/enforce_tier_firewall.mjs`)
...
`src/hooks/enforce_tier_firewall.mjs` so the firewall scans cleanly.
...
- `src/hooks/enforce_tier_firewall.mjs` — the PreToolUse firewall hook
