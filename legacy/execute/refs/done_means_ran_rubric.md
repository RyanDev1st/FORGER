...
walks before declaring done, and what `src/hooks/enforce_done_means_ran.mjs`
mirrors at the Stop hook.

If the rubric does not pass, the Stop hook **will** block. The hook is
registered in `src/hooks/settings.hooks.json` and reads
...
## How `src/hooks/enforce_done_means_ran.mjs` enforces this
...
5. **Computes missing criteria.** Any required criterion whose
   ...
   `suggested_action` of "Run src/gates/acceptance_test.mjs and fix any
...
- **Latest line wins.** A passing run followed by a failing run on
  ...
  `src/gates/acceptance_test.mjs` after every fix; do not rely on an
...
| Every claim in the report references a `claim_ledger` entry | DoW `hard_constraint` invoking `src/gates/audit.mjs --workspace <path>` (audit greps section claims against the ledger); hook enforces via step 5 |
...
| `src/gates/audit.mjs` passes when run against the report | Same `hard_constraint` row as above |
...
2. Did `src/gates/acceptance_test.mjs` exit 0 in this same session,
...
4. For research_report: did `src/gates/audit.mjs` exit 0 in this same
