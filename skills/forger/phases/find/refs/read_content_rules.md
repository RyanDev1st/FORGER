...
Companion reference for `src/lib/read_progressive.mjs`. Every page-content fetch
...
- `src/lib/read_progressive.mjs` — implementation.
- `skills/real_search/SKILL.md` — the skill that calls this library.
- `src/lib/playwright.mjs` — `headRequest` (used before SKIM) and the snapshot wrapper underneath SCAN/DEEP.
