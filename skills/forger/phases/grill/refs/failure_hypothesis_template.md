...
This is the canonical shape of a single entry in `workspaces/{slug}/failure_hypotheses.yaml`. The file itself is a YAML list; each list item is one entry. Validate with `src/lib/ledger.mjs::validateFailureHypothesis` (compiled from `schemas/failure_hypothesis.schema.yaml`). Fields below mirror the schema one-for-one.
...
```yaml
  ...
  # The provider string from src/lib/reviewer_router.mjs adapters: one of
...
- id: fh-backpressure-burst-regression
  ...
  what_disproves: |
    ...
    loop lag p95 <= 25 ms.
  minimal_test: |
    node src/cli/probe.mjs --workspace <path> --assumption-id fh-backpressure-burst-regression
