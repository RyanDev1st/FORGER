...
Only validated, working knowledge enters the KB:
  - proven_claim_ids must reference claims that:
    ...
    3. survived src/gates/audit.mjs without unresolved flags
...
**Criterion 3 — survived `src/gates/audit.mjs` without unresolved flags.**
...
- **`risk_map.yaml` assumptions with `status: probed_fail`.** These
  are assumptions OBSERVE tried to confirm with `src/cli/probe.mjs` and
...
## How each rule maps to `src/cli/update_kb.mjs`

`src/cli/update_kb.mjs` re-validates the retro on its way in, then:
