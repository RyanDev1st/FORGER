...
> `src/cli/update_kb.mjs` checks the last 3 telemetry lines after
...
`src/cli/update_kb.mjs`. A domain whose `last_updated_at` is 91 days
...
## Connection to `src/cli/update_kb.mjs`'s last-3 check

`src/cli/update_kb.mjs` performs the eligibility computation on every
